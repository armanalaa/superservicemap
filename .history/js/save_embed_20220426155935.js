/* ServiceMap.
  ...
  // var eventText= '';
   ...
   function save_handler(typeService, id, nameService, embed, textSearch) {
    ...
    var position = "#dialog";
    var dialog = "<div id='serviceMap_save_dialog' title='Save Information'><div   id='serviceMap_info_title' >You can copy or save this query on ServiceMap.<a id='sm_save_close' title='Close without save'>X</a></div>";
    if (save_operation == "write") {
      ...
      dialog+="<fieldset style='width: 505px;'><legend>Query</legend>";
      // dialog += "<b>JSON:</b>";
      ...
      if(type=='address' ||type=='event' || type=='path' || (type=="Position_Bus" && selectedorgUrl==superServiceMapApiUrl) || (typeService=="weather" && selectedorgUrl==superServiceMapApiUrl)){
      ...
      if(format=='json'){
        dialog+="<div id='save_json_div'>";
        dialog+="<fieldset style='width: 505px;'><legend id='legend_format'>JSON Format</legend>";
        ...
        dialog+="<fieldset  style='width: 505px;'><legend id='legend_format'>HTML Format</legend>";
            ...
            xhr_org_selector.onload = function () {
                if (this.status === 200) {
                    ...
                    for (var i = 0; i <= obj.length - 1; i++) {
                        ...
                        if(org.toLowerCase()==organizationName.toLowerCase()){
                          orgUrl=kbUrl;
                        }
                    }
                } else {
...
function copyText(text) {
      ...
      //-----------------------------------------------FINE EMBED DA SISTEMARE    
          ...
          if(type=='event'){
              ...
              if (id != null && id!="-1") {
                ...
                if(servicecTextFilter!="")    
                  link += "&text=" + encodeURIComponent(servicecTextFilter);
 ...
 function saveQueryInKB(stringaCategorie,numeroRisultatiServizi,numeroRisultatiSensori,numeroRisultatiBus,coordinateSelezione,raggioServizi,raggioSensori,raggioBus,actualSelection,weatherCity,stringaPopupOpen,zoom,center,parentQuery,typeService,id,nameService,type,text) {  
      ...
      // var idQuery = Date.now().toString();
      // var idConf = Date.now().toString();

      // var idQuery_md5 = md5(idQuery);
      // var idConf_md5 = md5(idConf);
...
accessToken:accessToken
  ...
  function saveQueryServices(centroRicerca, raggio, categorie, numeroRisultati, serviceName, text, serviceModel) {
